...
[Quick Start](#-quick-start) · [Demo Apps](#-nova-demo-apps) · [Roadmap](#-roadmap) · [Documentation](#-documentation)
...
- **Privacy Guard** — Regex-based PII detection across 6 types
- **Action Router** — Policy-enforced native action execution
- **Feedback Loop** — Event tracking across the pipeline
- **Readiness Scoring** — 7-dimension quality measurement from 0 to 100
- **Platform Adapters** — Native implementations for React Native, Android, and iOS
...
- 🔒 **Privacy Mode** — Test PII detection on email, phone, tokens, and more
...
- iOS Simulator on Mac or Android Emulator
...
console.log(result.readinessScore); // 72 → Good
```
...
- ❌ **No real LLM provider integration** — responses are deterministic
- ❌ **In-memory storage only** — no Room, DataStore, CoreData, or SwiftData persistence yet
...
- ✅ Privacy guard with 6 PII types
...
**Note:** v0.2 is not a React Native Fabric bridge. It is a native Android implementation proving the architecture is platform-agnostic.
